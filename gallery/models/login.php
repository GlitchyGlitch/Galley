...
class LoginInput extends Model implements ModelInterface
{
  ...
  public function validate_email(): bool
  {
    ...
    if (!(strlen($this->email) > 320) && filter_var($this->email, FILTER_VALIDATE_EMAIL)) {
    }
    return false;
  }
  public function validate_password(): bool
  {
    if (!(strlen($this->password) < 8) && !(strlen($this->password) > 256) && !preg_match('/[^\x20-\x7f]/', $this->password)) {
      return true;
    }
    return false;
  }
